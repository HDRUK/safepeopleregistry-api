...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        if ($config) {
            $oldValue = json_decode($config->value, true)['validationSchema'];

            $updatedSchema = array_merge($oldValue, self::validationSchema);
            $config->value = ['validationSchema' => $updatedSchema];        
            $config->save();
        }
    }
    ...
    public function down(): void
    {
        ...
        if ($config) {
            $schema = json_decode($config->value, true);
